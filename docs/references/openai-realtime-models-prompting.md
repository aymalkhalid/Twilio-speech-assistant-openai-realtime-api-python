...
> **Source:** [Using realtime models](https://developers.openai.com/api/docs/guides/realtime-models-prompting) (OpenAI API docs)
> **Captured:** 2026-06-20
> **Use in this repo:** Reference when editing `prompts/main_system_instructions.md`, tool schemas in `services/openai_service.py`, and session config in `config.py`.

This is a local copy of OpenAI's realtime prompting guidance for offline reference and prompt iteration. Prefer the live docs for the latest updates.

## Table of contents

### Realtime 2.0 (`gpt-realtime-2`)
- [Choose a model](#choose-a-model)
- [What changed in Realtime 2](#what-changed-in-realtime-2)
- [Recommended prompt structure](#recommended-prompt-structure)
- [Set reasoning effort](#set-reasoning-effort)
- [Use preambles intentionally](#use-preambles-intentionally)
- [Control response length](#control-response-length)
- [Design tool behavior](#design-tool-behavior)
- [Handle silence and background audio](#handle-silence-and-background-audio)
- [Use message channels deliberately](#use-message-channels-deliberately)
- [Handle unclear audio](#handle-unclear-audio)
- [Capture exact entities](#capture-exact-entities)
- [Avoid literal instruction traps](#avoid-literal-instruction-traps)
- [Control language and accent separately](#control-language-and-accent-separately)
- [Maintain state in long sessions](#maintain-state-in-long-sessions)
- [Migrate from earlier realtime models](#migrate-from-earlier-realtime-models)

### Realtime 1.5 (`gpt-realtime-1.5`)
...
### Starter mapping
See [STARTER_PROMPT_MAPPING.md](./STARTER_PROMPT_MAPPING.md) for how this repo's prompt aligns with the guide.
...
| --- | --- | --- |
| [`gpt-realtime-2`](https://developers.openai.com/api/docs/models/gpt-realtime-2) | You need the strongest realtime reasoning, tool use, and instruction following. | Tune reasoning effort, preambles, tool policies, exact entity capture, and long-session state. |
| [`gpt-realtime-1.5`](https://developers.openai.com/api/docs/models/gpt-realtime-1.5) | You need a fast, reliable non-reasoning speech-to-speech model. | Follow the core realtime prompt structure and test for latency-sensitive behavior. |
...
- Review the earlier Realtime prompting guide for more `gpt-realtime-1.5` examples.
- Review the Realtime eval guide to test representative voice-agent behavior.
- Learn how to connect with WebRTC, WebSocket, or SIP.
- Learn the Realtime conversation lifecycle.
- Review Realtime costs.